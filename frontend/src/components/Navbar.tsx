...
export const Navbar: React.FC<NavbarProps> = ({ activeSection, scrollToSection }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 liquid-glass-nav">
      ...
      <div 
        className="absolute top-0 left-0 h-[2.5px] bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 transition-all duration-100" 
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <button 
          ...
          className="md:hidden text-[#8a8a93] hover:text-white cursor-pointer"
        >
      ...
      {isOpen && (
        <div className="md:hidden absolute top-16 left-0 right-0 bg-[#030303]/95 border-b border-white/5 py-4 px-6 flex flex-col space-y-3 z-45">
          {navLinks.map((link) => (
            <button
              key={link.id}
              onClick={() => {
